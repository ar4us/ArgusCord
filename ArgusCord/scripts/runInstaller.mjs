...
const BASE_URL = "https://github.com/ArgusCord/Installer/releases/latest/download/";
const INSTALLER_PATH_DARWIN = "ArgusCordInstaller.app/Contents/MacOS/ArgusCordInstaller";
...
function getFilename() {
    switch (process.platform) {
        case "win32":
            return "ArgusCordInstallerCli.exe";
        case "darwin":
            return "ArgusCordInstaller.MacOS.zip";
        case "linux":
            return "ArgusCordInstallerCli-linux";
        default:
...
async function ensureBinary() {
    ...
    const outputFile = process.platform === "darwin"
        ? join(FILE_DIR, "ArgusCordInstaller")
        : downloadName;
    ...
    const res = await fetch(BASE_URL + filename, {
        headers: {
            "User-Agent": "ArgusCord (https://github.com/ar4us/ArgusCord)",
            "If-None-Match": etag
        }
    });
    ...
    if (process.platform === "darwin") {
        ...
        logAndRun(`sudo spctl --add '${outputFile}' --label "ArgusCord Installer"`);
...
try {
    execFileSync(installerBin, args, {
        ...
        env: {
            ...process.env,
            ARGUSCORD_USER_DATA_DIR: BASE_DIR,
            ARGUSCORD_DEV_INSTALL: "1"
        }
    });
} catch {
