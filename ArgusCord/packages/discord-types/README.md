...
While it was primarily created for ArgusCord, other client mods could also benefit from this, so it is published as a standalone package!

## Installation

```bash
npm install -D @arguscord/discord-types
yarn add -D @arguscord/discord-types
pnpm add -D @arguscord/discord-types
```

## Example Usage

```ts
import type { UserStore } from "@arguscord/discord-types";
...
```ts
import { ApplicationCommandType } from "@arguscord/discord-types/enums";
