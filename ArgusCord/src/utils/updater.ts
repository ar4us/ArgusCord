...
export async function checkForUpdates() {
    changes = await Unwrap(ArgusCordNative.updater.getUpdates());
...
export async function update() {
    ...
    const res = await Unwrap(ArgusCordNative.updater.update());

    if (res) {
        isOutdated = false;
        if (!await Unwrap(ArgusCordNative.updater.rebuild()))
...
export const getRepo = () => Unwrap(ArgusCordNative.updater.getRepo());
