    ...
    try {
        ...
        addPatch({
            ...
            replacement: {
                ...
                replace: "(ArgusCord.Webpack._initReporter(),$&)"
            }
        }, "ArgusCord Reporter");

        // @ts-expect-error
        ArgusCord.Webpack._initReporter = function () {
        ...
        for (const [searchType, args] of Webpack.lazyWebpackSearchHistory) {
            ...
            try {
                ...
                if (result == null || (result.$$arguscordGetWrappedComponent != null && result.$$arguscordGetWrappedComponent() == null)) throw new Error("Webpack Find Fail");
            } catch (e) {
                let logMessage = searchType;
                if (method === "find" || method === "proxyLazyWebpack" || method === "LazyComponentWebpack") {
                    if (args[0].$$arguscordProps != null) {
                        logMessage += `(${args[0].$$arguscordProps.map(arg => `"${arg}"`).join(", ")})`;
                    } else {
...
// Run after the ArgusCord object has been created.
// We need to add extra properties to it, and it is only created after all of ArgusCord code has ran
