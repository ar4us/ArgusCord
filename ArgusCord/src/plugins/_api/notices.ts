            ...
            replacement: [
                {
                    ...
                    replace: "if(ArgusCord.Api.Notices.currentNotice)return false;$&"
                },
                {
                    match: /(?<=,NOTICE_DISMISS:function\(\i\){)return null!=(\i)/,
                    replace: (m, notice) => `if(${notice}?.id=="ArgusCordNotice")return(${notice}=null,ArgusCord.Api.Notices.nextNotice(),true);${m}`
                }
            ]
        }
    ],
