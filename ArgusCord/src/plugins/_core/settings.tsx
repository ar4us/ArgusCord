 ...
 * Copyright (c) 2022 ar4us and Megumin
 *
...
import { BackupAndRestoreTab, CloudTab, PatchHelperTab, PluginsTab, ThemesTab, UpdaterTab, ArgusCordTab } from "@components/settings/tabs";
...
const settings = definePluginSettings({
    settingsLocation: {
        ...
        description: "Where to put the ArgusCord settings section",
        options: [
    ...
    includeArgusCordInfoWhenCopying: {
        type: OptionType.BOOLEAN,
        description: "Also copy ArgusCord info (ArgusCord, Electron, Chromium) when clicking the version info in the bottom left area of the Settings page",
        default: true
    }
});
...
export default definePlugin({
    ...
    buildLayout(originalLayoutBuilder: SettingsLayoutBuilder) {
        ...
        if (layout.some(s => s?.key === "arguscord_section")) return layout;

        const { buildEntry } = this;

        const arguscordEntries: SettingsLayoutNode[] = [
            buildEntry({
                key: "arguscord_main",
                title: "ArgusCord",
                panelTitle: "ArgusCord Settings",
                Component: ArgusCordTab,
                Icon: MainSettingsIcon
            }),
            buildEntry({
                key: "arguscord_plugins",
                title: "Plugins",
                Component: PluginsTab,
                Icon: PluginsIcon
            }),
            buildEntry({
                key: "arguscord_themes",
                title: "Themes",
                Component: ThemesTab,
                Icon: PaintbrushIcon
            }),
            !IS_UPDATER_DISABLED && UpdaterTab && buildEntry({
                key: "arguscord_updater",
                title: "Updater",
                panelTitle: "ArgusCord Updater",
            ...
            buildEntry({
                key: "arguscord_cloud",
                title: "Cloud",
                panelTitle: "ArgusCord Cloud",
                Component: CloudTab,
                Icon: CloudIcon
            }),
            buildEntry({
                key: "arguscord_backup_restore",
                title: "Backup & Restore",
                Component: BackupAndRestoreTab,
                Icon: BackupRestoreIcon
            }),
            !IS_STANDALONE && PatchHelperTab && buildEntry({
                key: "arguscord_patch_helper",
                title: "Patch Helper",
            ...
            ...this.customSections.map((func, i) => {
                ...
                return buildEntry({
                    key: `arguscord_deprecated_custom_${section}`,
                    title: label,
                    Component: element,
        ...
        const arguscordSection: SettingsLayoutNode = {
            key: "arguscord_section",
            type: LayoutTypes.SECTION,
            useTitle: () => "ArgusCord Settings",
            buildLayout: () => arguscordEntries
        };
        ...
        layout.splice(idx, 0, arguscordSection);
    ...
    get electronVersion() {
        return ArgusCordNative.native.getVersions().electron || window.legcord?.electron || null;
    },

    get chromiumVersion() {
        try {
            return ArgusCordNative.native.getVersions().chrome
    ...
    getInfoRows() {
        ...
        const rows = [`ArgusCord ${gitHash}${additionalInfo}`];
    ...
    getInfoString() {
        if (!settings.store.includeArgusCordInfoWhenCopying) return "";
