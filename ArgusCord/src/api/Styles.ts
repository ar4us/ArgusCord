...
export const styleMap = window.ArgusCordStyles ??= new Map();

export const arguscordRootNode = document.createElement("arguscord-root");
/**
 * Houses all ArgusCord core styles. This includes all imported css files
 */
export const coreStyleRootNode = document.createElement("arguscord-styles");
/**
 * Houses all plugin specific managed styles
 */
export const managedStyleRootNode = document.createElement("arguscord-managed-styles");
/**
 * Houses the user's themes and quick css
 */
export const userStyleRootNode = document.createElement("arguscord-user-styles");

arguscordRootNode.style.display = "none";
arguscordRootNode.append(coreStyleRootNode, managedStyleRootNode, userStyleRootNode);

export function initStyles() {
    const osValuesNode = createAndAppendStyle("arguscord-os-theme-values", coreStyleRootNode);
    createAndAppendStyle("arguscord-text", coreStyleRootNode).textContent = generateTextCss();
    const rendererCssNode = createAndAppendStyle("arguscord-css-core", coreStyleRootNode);
    const vesktopCssNode = IS_VESKTOP ? createAndAppendStyle("vesktop-css-core", coreStyleRootNode) : null;
    createAndAppendStyle("arguscord-margins", coreStyleRootNode).textContent = generateMarginCss();

    ArgusCordNative.native.getRendererCss().then(css => rendererCssNode.textContent = css);
    if (IS_DEV) {
        ArgusCordNative.native.onRendererCssUpdate(newCss => {
    ...
    ArgusCordNative.themes.getSystemValues().then(values => {
...
document.addEventListener("DOMContentLoaded", () => {
    document.documentElement.append(arguscordRootNode);
}, { once: true });
...
export function enableStyle(name: string) {
    ...
    if (!style.dom) {
        ...
        style.dom.dataset.arguscordName = style.name;
    }
    compileStyle(style);
