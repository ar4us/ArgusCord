...
import { CommandArgument, CommandContext, CommandOption } from "@arguscord/discord-types";

import { sendBotMessage } from "./commandHelpers";
import { ApplicationCommandInputType, ApplicationCommandOptionType, ApplicationCommandType, ArgusCordCommand } from "./types";

export * from "./commandHelpers";
export * from "./types";

export let BUILT_IN: ArgusCordCommand[];
export const commands = {} as Record<string, ArgusCordCommand>;
...
export const _init = function (cmds: ArgusCordCommand[]) {
    try {
        BUILT_IN = cmds;
...
export const _handleCommand = function (cmd: ArgusCordCommand, args: CommandArgument[], ctx: CommandContext) {
    if (!cmd.isArgusCordCommand)
 ...
 * @param opt
 */
export function prepareOption<O extends CommandOption | ArgusCordCommand>(opt: O): O {
...
// investigate
function registerSubCommands(cmd: ArgusCordCommand, plugin: string) {
...
export function registerCommand<C extends ArgusCordCommand>(command: C, plugin: string) {
    if (!BUILT_IN) {
        console.warn(
            "[CommandsAPI]",
    ...
    command.isArgusCordCommand = true;
