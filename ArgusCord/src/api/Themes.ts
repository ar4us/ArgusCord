...
import { ThemeStore } from "@arguscord/discord-types";
import { PopoutWindowStore } from "@webpack/common";

import { userStyleRootNode, arguscordRootNode } from "./Styles";
...
async function toggle(isEnabled: boolean) {
    if (!style) {
        if (isEnabled) {
            style = createAndAppendStyle("arguscord-custom-css", userStyleRootNode);
            ArgusCordNative.quickCss.addChangeListener(css => {
                style.textContent = css;
                // At the time of writing this, changing textContent resets the disabled state
                style.disabled = !Settings.useQuickCss;
                updatePopoutWindows();
            });
            style.textContent = await ArgusCordNative.quickCss.get();
        }
    } else
...
async function initThemes() {
    themesStyle ??= createAndAppendStyle("arguscord-themes", userStyleRootNode);
    ...
    if (IS_WEB) {
        for (const theme of enabledThemes) {
            const themeData = await ArgusCordNative.themes.getThemeData(theme);
            if (!themeData) continue;
            const blob = new Blob([themeData], { type: "text/css" });
            links.push(URL.createObjectURL(blob));
        }
    } else {
        const localThemes = enabledThemes.map(theme => `arguscord:///themes/${theme}?v=${Date.now()}`);
...
function applyToPopout(popoutWindow: Window | undefined, key: string) {
    ...
    doc.querySelector("arguscord-root")?.remove();

    doc.documentElement.appendChild(arguscordRootNode.cloneNode(true));
}
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (!IS_WEB) {
        ArgusCordNative.quickCss.addThemeChangeListener(initThemes);
    }
}, { once: true });
