...
func GetInstallerDownloadLink() string {
	const BaseUrl = "https://github.com/ArgusCord/Installer/releases/latest/download/"
	switch runtime.GOOS {
	case "windows":
		filename := Ternary(buildinfo.UiType == buildinfo.UiTypeCli, "ArgusCordInstallerCli.exe", "ArgusCordInstaller.exe")
		return BaseUrl + filename
	case "darwin":
		return BaseUrl + "ArgusCordInstaller.MacOS.zip"
	case "linux":
		return BaseUrl + "ArgusCordInstallerCli-linux"
	default:
		return ""
	}
}
...
func UpdateSelf() error {
	...
	tmp, err := os.CreateTemp(ownExeDir, "ArgusCordInstallerUpdate")
	if err != nil {
