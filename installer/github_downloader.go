...
func GetGithubRelease(url, fallbackUrl string) (*GithubRelease, error) {
	...
	if res.StatusCode >= 300 {
		...
		// If that is the case, try our fallback at https://arguscord.dev/releases/project
...
func InitGithubDownloader() {
	...
	IsDevInstall = os.Getenv("ARGUSCORD_DEV_INSTALL") == "1"
	...
	Log.Debug("Found existing ArgusCord Install. Checking for hash...")
	scanner := bufio.NewScanner(f)
	if scanner.Scan() {
		line := scanner.Text()
		if strings.HasPrefix(line, "// ArgusCord ") {
