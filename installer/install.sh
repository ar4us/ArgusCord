...
if grep -q "CHROMEOS_RELEASE_NAME" /etc/lsb-release 2>/dev/null; then
	echo "ChromeOS is not supported! Use the chrome extension. https://chromewebstore.google.com/detail/arguscord-web/cbghhgpcnddeihccjmnadmkaejncjndb"
	exit 1
fi
...
curl -sS https://github.com/Vendicated/ArgusCordInstaller/releases/latest/download/ArgusCordInstallerCli-Linux \
  --output "$outfile" \
  --location \
  --fail
