...
func main() {
	...
	var installFlag = flag.Bool("install", false, "Install ArgusCord")
	var updateFlag = flag.Bool("repair", false, "Repair ArgusCord")
	var uninstallFlag = flag.Bool("uninstall", false, "Uninstall ArgusCord")
	...
	if *versionFlag {
		fmt.Println("ArgusCord Installer Cli", buildinfo.InstallerTag, "("+buildinfo.InstallerGitHash+")")
		fmt.Println("Copyright (C) 2023 Vendicated and ArgusCord contributors")
	...
	if !SliceContainsFunc(switches, func(b *bool) bool { return *b }) {
		...
		go func() {
			...
			if IsSelfOutdated {
				...
				Log.Warn("To update, select the 'Update ArgusCord Installer' option to update, or run with --update-self")
			}
		}()
		...
		case "Quit":
			return
		case "Update ArgusCord Installer":
			if err := UpdateSelf(); err != nil {
				Log.Error("Failed to update self:", err)
				exitFailure()
			}
	...
	if install {
		...
		Log.Info("Downloading latest ArgusCord files...")
...
func HandleScuffedInstall() {
	...
	fmt.Println("Otherwise, ArgusCord will likely not work.")
}
