...
func init() {
	if dir := os.Getenv("ARGUSCORD_USER_DATA_DIR"); dir != "" {
		Log.Debug("Using ARGUSCORD_USER_DATA_DIR")
		BaseDir = dir
	} else if dir = os.Getenv("DISCORD_USER_DATA_DIR"); dir != "" {
		Log.Debug("Using DISCORD_USER_DATA_DIR/../ArgusCordData")
		BaseDir = path.Join(dir, "..", "ArgusCordData")
	} else {
		Log.Debug("Using UserConfig")
		BaseDir = appdir.New("ArgusCord").UserConfig()
	}
