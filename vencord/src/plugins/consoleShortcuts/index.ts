...
function makeShortcuts() {
    ...
    return {
        ...
        wpPatcher: { getter: () => ArgusCord.WebpackPatcher },
        wpInstances: { getter: () => ArgusCord.WebpackPatcher.allWebpackInstances },
        wpsearch: search,
        wpex: extract,
        ...
        PluginsApi: { getter: () => ArgusCord.Plugins },
        plugins: { getter: () => ArgusCord.Plugins.plugins },
        Settings: { getter: () => ArgusCord.Settings },
        Api: { getter: () => ArgusCord.Api },
        Util: { getter: () => ArgusCord.Util },
            ...
            if (!win.prepared) {
                ...
                [...document.querySelectorAll("style"), ...document.querySelectorAll("link[rel=stylesheet]")].forEach(s => {
                    ...
                    if (s.parentElement?.tagName === "HEAD")
                        doc.head.append(n);
                    else if (n.id?.startsWith("arguscord-") || n.id?.startsWith("vcd-"))
        ...
        preEnable: (plugin: string) => (ArgusCord.Settings.plugins[plugin] ??= { enabled: true }).enabled = true,
...
function loadAndCacheShortcut(key: string, val: any, forceLoad: boolean) {
    ...
    function unwrapProxy(value: any) {
        ...
        } else if (value.$$arguscordGetWrappedComponent) {
            return forceLoad ? value.$$arguscordGetWrappedComponent() : value;
        }
...
export default definePlugin({
    ...
    start() {
        ...
        if (!IS_WEB) {
            const Native = ArgusCordNative.pluginHelpers.ConsoleShortcuts as PluginNative<typeof import("./native")>;
