...
import { CONTRIB_ROLE_ID, Devs, DONOR_ROLE_ID, KNOWN_ISSUES_CHANNEL_ID, REGULAR_ROLE_ID, SUPPORT_CATEGORY_ID, SUPPORT_CHANNEL_ID, VENBOT_USER_ID, ARGUSCORD_GUILD_ID } from "@utils/constants";
...
import { Channel, RenderModalProps } from "@arguscord/discord-types";
...
const AdditionalAllowedChannelIds = [
    "1024286218801926184", // ArgusCord > #bot-commands
];
...
async function generateDebugInfoMessage() {
    ...
    const info = {
        ArgusCord:
            `v${VERSION} • [${gitHash}](<https://github.com/Vendicated/ArgusCord/commit/${gitHash}>)` +
    ...
    const commonIssues = {
        ...
        "ArgusCord DevBuild": !IS_STANDALONE,
...
function DevBuildConfirmModal(props: RenderModalProps) {
    ...
    return (
        ...
        >
            <div>
                <Forms.FormText>You are using a custom build of ArgusCord, which we do not provide support for!</Forms.FormText>

                <Forms.FormText className={Margins.top8}>
                    We only provide support for <Link href="https://arguscord.dev/download">official builds</Link>.
                    Either <Link href="https://arguscord.dev/download">switch to an official build</Link> or figure your issue out yourself.
                </Forms.FormText>
...
export default definePlugin({
    ...
    commands: [
        {
            name: "arguscord-debug",
            description: "Send ArgusCord debug info",
            predicate: ctx => isPluginDev(UserStore.getCurrentUser()?.id) || isSupportAllowedChannel(ctx.channel),
            execute: async () => ({ content: await generateDebugInfoMessage() })
        },
        {
            name: "arguscord-plugins",
            description: "Send ArgusCord plugin list",
            predicate: ctx => isPluginDev(UserStore.getCurrentUser()?.id) || isSupportAllowedChannel(ctx.channel),
    ...
    flux: {
        async CHANNEL_SELECT({ channelId }) {
            ...
            if (!IS_UPDATER_DISABLED) {
                ...
                if (isOutdated) {
                    openModal(props => (
                        ...
                        >
                            <div>
                                <Forms.FormText>You are using an outdated version of ArgusCord! Chances are, your issue is already fixed.</Forms.FormText>
                                <Forms.FormText className={Margins.top8}>
            ...
            const roles = GuildMemberStore.getSelfMember(ARGUSCORD_GUILD_ID)?.roles;
            ...
            if (!IS_WEB && IS_UPDATER_DISABLED) {
                openModal(props => (
                    ...
                    >
                        <div>
                            <Forms.FormText>You are using an externally updated ArgusCord version, which we do not provide support for!</Forms.FormText>
                            <Forms.FormText className={Margins.top8}>
                                Please either switch to an <Link href="https://arguscord.dev/download">officially supported version of ArgusCord</Link>, or
    ...
    renderMessageAccessory(props) {
        ...
        if (props.channel.parent_id === SUPPORT_CATEGORY_ID && PermissionStore.can(PermissionsBits.SEND_MESSAGES, props.channel)) {
            if (props.message.content.includes("/arguscord-debug") || props.message.content.includes("/arguscord-plugins")) {
                buttons.push(
                    <Button
                        key="vc-dbg"
    ...
    renderContributorDmWarningCard: ErrorBoundary.wrap(({ channel }) => {
        ...
        return (
            <Card variant="warning" className={Margins.top8} defaultPadding>
                Please do not private message ArgusCord plugin developers for support!
                <br />
                Instead, use the ArgusCord support channel: {Parser.parse("https://discord.com/channels/1015060230222131221/1026515880080842772")}
