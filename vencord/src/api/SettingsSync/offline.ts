...
export async function importSettings(data: string) {
    ...
    if ("settings" in parsed && "quickCss" in parsed) {
        ...
        await ArgusCordNative.settings.set(parsed.settings);
        await ArgusCordNative.quickCss.set(parsed.quickCss);
    } else
        throw new Error("Invalid Settings. Is this even a ArgusCord Settings file?");
}

export async function exportSettings({ minify }: { minify?: boolean; } = {}) {
    const settings = ArgusCordNative.settings.get();
    const quickCss = await ArgusCordNative.quickCss.get();
    return JSON.stringify({ settings, quickCss }, null, minify ? undefined : 4);
}

export async function downloadSettingsBackup() {
    const filename = `arguscord-settings-backup-${moment().format("YYYY-MM-DD")}.json`;
...
export async function uploadSettingsBackup(showToast = true): Promise<void> {
    if (IS_DISCORD_DESKTOP) {
        const [file] = await DiscordNative.fileManager.openFiles({
            filters: [
                { name: "ArgusCord Settings Backup", extensions: ["json"] },
                { name: "all", extensions: ["*"] }
            ]
        });
