...
export async function checkCloudUrlCsp() {
    ...
    if (host === "api.arguscord.dev") return true;

    if (await ArgusCordNative.csp.isDomainAllowed(Settings.cloud.url, ["connect-src"])) {
        return true;
    }

    const res = await ArgusCordNative.csp.requestAddOverride(Settings.cloud.url, ["connect-src"], "Cloud Sync");
    if (res === "ok") {
        openModal(props => (
            <ConfirmModal
                {...props}
...
export async function getAuthorization() {
    const secrets = await DataStore.get<Record<string, string>>("ArgusCord_cloudSecret") ?? {};

    const origin = getCloudUrlOrigin();

    // we need to migrate from the old format here
    if (secrets[origin]) {
        await DataStore.update<Record<string, string>>("ArgusCord_cloudSecret", secrets => {
            secrets ??= {};
