...
// Only import pure modules that don't import other parts of ArgusCord.
...
const setCssDebounced = debounce((css: string) => ArgusCordNative.quickCss.set(css));

const themeStore = DataStore.createStore("ArgusCordThemes", "ArgusCordThemeData");

// probably should make this less cursed at some point
window.ArgusCordNative = {
    themes: {
    ...
    updater: {
        getRepo: async () => ({ ok: true, value: "https://github.com/Vendicated/ArgusCord" }),
    ...
    quickCss: {
        get: () => DataStore.get("ArgusCordQuickCss").then(s => s ?? ""),
        set: async (css: string) => {
            await DataStore.set("ArgusCordQuickCss", css);
        ...
        async openEditor() {
            ...
            const win = open("about:blank", "ArgusCordQuickCss", features);
            if (!win) {
            ...
            win.getCurrentCss = () => ArgusCordNative.quickCss.get();
    ...
    settings: {
        get: () => {
            try {
                return JSON.parse(localStorage.getItem("ArgusCordSettings") || "{}");
            } catch (e) {
                console.error("Failed to parse settings from localStorage: ", e);
                return {};
            }
        },
        set: async (s: Settings) => localStorage.setItem("ArgusCordSettings", JSON.stringify(s)),
