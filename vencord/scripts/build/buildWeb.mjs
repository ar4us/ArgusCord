...
const commonOptions = {
    ...commonOpts,
    entryPoints: ["browser/ArgusCord.ts"],
    format: "iife",
...
const buildConfigs = [
    ...
    },
    {
        ...
        footer: { js: "//# sourceURL=file:///ArgusCordWeb" }
    },
    {
        ...commonOptions,
    ...
    },
    {
        ...
        outfile: "dist/ArgusCord.user.js",
        banner: {
            js: readFileSync("browser/userscript.meta.js", "utf-8").replace("%version%", `${VERSION}.${new Date().getTime()}`)
        },
        footer: {
            // UserScripts get wrapped in an iife, so define ArgusCord prop on window that returns our local
            js: "Object.defineProperty(unsafeWindow,'ArgusCord',{get:()=>ArgusCord});"
        }
    }
];
...
async function buildExtension(target, files) {
    const entries = {
        "dist/ArgusCord.js": await readFile("dist/extension.js"),
        "dist/ArgusCord.css": await readFile("dist/extension.css"),
...
const appendCssRuntime = readFile("dist/ArgusCord.user.css", "utf-8").then(content => {
    const cssRuntime = `unsafeWindow._vcUserScriptRendererCss=\`${content.replaceAll("`", "\\`")}\``;

    return appendFile("dist/ArgusCord.user.js", cssRuntime);
});
